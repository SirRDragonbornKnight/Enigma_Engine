...
// confirm any pick on the live overlay (cascade philosophy: identify by structure, not by name).
//
...
function decodeVec3(gltf, bin, ai) {
  ...
  if (acc.sparse) {
    // sparse = only changed verts (very common for morphs)
    try {
      const s = acc.sparse,
      ...
      const ibase = (iv.byteOffset || 0) + (s.indices.byteOffset || 0),
        vbase = (vv.byteOffset || 0) + (s.values.byteOffset || 0);
      const ict = s.indices.componentType,
        isz = ict === 5125 ? 4 : ict === 5123 ? 2 : 1;
      const readIdx =
        ict === 5125 ? (o) => bin.readUInt32LE(o) : ict === 5123 ? (o) => bin.readUInt16LE(o) : (o) => bin.readUInt8(o);
      for (let k = 0; k < s.count; k++) {
        const idx = readIdx(ibase + k * isz),
          vo = vbase + k * 12;
        out[idx * 3] = bin.readFloatLE(vo);
        out[idx * 3 + 1] = bin.readFloatLE(vo + 4);
        out[idx * 3 + 2] = bin.readFloatLE(vo + 8);
      }
    } catch {
      /* malformed sparse — keep the (zero) base rather than crash */
    }
  }
  return out;
}
...
export function scan(file) {
  ...
  for (const m of gltf.meshes || []) {
    for (const pr of m.primitives || []) {
      if (pr.extensions?.KHR_draco_mesh_compression) {
        draco = true;
        continue;
      }
      ...
      let baseP, deltas;
      try {
        baseP = decodeVec3(gltf, bin, pr.attributes.POSITION);
        deltas = pr.targets.map((t) => (t.POSITION != null ? decodeVec3(gltf, bin, t.POSITION) : null));
      } catch {
        continue;
      } // one bad accessor must not abort the whole scan
      if (!baseP) continue;
  ...
  if (!meshes.length)
    return {
      ...
      error: draco
        ? "Draco-compressed — decode LIVE (geom_mouth.js + DRACOLoader); offline scan can't read it"
        : "no decodable morph targets (none, or quantized positions)",
    };
  ...
  const score = new Array(nMorph).fill(0),
    info = new Array(nMorph).fill(null);
  for (let mi = 0; mi < nMorph; mi++) {
    let down = 0,
      cnt = 0,
      usum = 0;
    for (const ms of meshes) {
      const d = ms.deltas[mi];
      if (!d) continue;
      for (let v = 0; v < ms.count; v++) {
        const bu = ms.base[v * 3 + up];
        if (bu < headCut) continue; // HEAD region (along up)
        const du = d[v * 3 + up];
        if (du >= 0) continue; // downward = jaw drop
        const mag = -du;
    ...
    score[mi] = down;
    info[mi] = {
      downScore: +down.toFixed(3),
      verts: cnt,
      regCenterNormUp: cnt ? +((usum / cnt - mn[up]) / H).toFixed(2) : null,
    };
  }
  const upAxis = "XYZ"[up];
  ...
  return {
    ...
    note: "pick = strongest downward (jaw-drop) motion in the head region, gated by near-equal count (same rule as geom_mouth.js). Verify live.",
  };
}
