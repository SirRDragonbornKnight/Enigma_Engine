@echo off
:: Enigma AI Engine Launcher for Windows
:: Double-click this to start Enigma AI Engine

title Enigma AI Engine
echo.
echo  ███████╗ ██████╗ ██████╗  ██████╗ ███████╗     █████╗ ██╗
echo  ██╔════╝██╔═══██╗██╔══██╗██╔════╝ ██╔════╝    ██╔══██╗██║
echo  █████╗  ██║   ██║██████╔╝██║  ███╗█████╗      ███████║██║
echo  ██╔══╝  ██║   ██║██╔══██╗██║   ██║██╔══╝      ██╔══██║██║
echo  ██║     ╚██████╔╝██║  ██║╚██████╔╝███████╗    ██║  ██║██║
echo  ╚═╝      ╚═════╝ ╚═╝  ╚═╝ ╚═════╝ ╚══════╝    ╚═╝  ╚═╝╚═╝
echo.
echo  Starting Enigma AI Engine...
echo.

:: Change to script directory
cd /d "%~dp0"

:: Check if Python is available
python --version >nul 2>&1
if errorlevel 1 (
    echo [ERROR] Python is not installed or not in PATH.
    echo.
    echo Please install Python 3.9+ from https://python.org
    echo Make sure to check "Add Python to PATH" during installation.
    echo.
    pause
    exit /b 1
)

:: Check if dependencies are installed (quick check for torch)
python -c "import torch" >nul 2>&1
if errorlevel 1 (
    echo [INFO] Installing dependencies... This may take a few minutes.
    echo.
    pip install -r requirements.txt
    if errorlevel 1 (
        echo [ERROR] Failed to install dependencies.
        pause
        exit /b 1
    )
)

:: Launch Enigma AI Engine GUI
echo [INFO] Launching Enigma AI Engine...

:: Try pythonw first (no terminal window) - if it fails, fall back to python
pythonw run.py --gui 2>nul
if errorlevel 1 (
    :: pythonw failed or not available, use regular python
    python run.py --gui
)

:: If we get here, the app closed
if errorlevel 1 (
    echo.
    echo [ERROR] Enigma AI Engine encountered an error.
    pause
)
