...
const conjurer = createConjure({   // P3: spawn props + move them with cartoon transforms (rapier stays for throw/drop)
  scene, loadAsset,
  getBoneWorld: (role) => { const r = proc?.roles?.(); const b = r && r[role]; return b ? b.getWorldPosition(new THREE.Vector3()) : null; },
  onMiss: (url) => { setStatus("conjure: couldn't load " + url); console.warn("[avatar] conjure: asset failed to load:", url); },   // surface a missing/bad prop instead of vanishing silently
});
...
function surfaceYAt(gx, gy) {                    // nearest AI-placed PLATFORM top under gx, within the snap band
...
function floorSnap() {                           // drag-release / glide-arrival: feet ease onto a nearby PLATFORM top (screen-bottom floor snap removed 2026-06-25)
...
function disposeModel() {
  ...
  model = null; vrm = null; proc = null; spring = null; _meshList = null;
}
...
function enterNoModel() {
  ...
  curKey = DEFAULT_KEY;
  modelDims = { w: 1, h: 2 };
  roleBones = {}; eyeBones = []; _weightMass = null; _springNeverExtra = [];
  _poseBones = []; _poseMorphs = []; poseLen = 0; _poseBuf = null; _lastPose = null;
  facial = null; proc = null; spring = null; vrm = null;
  hitMask = null;
...
function _buildLoadedModel(asset) {
  ...
  setStatus(`loaded ✓ ${resolved.matched.length ? resolved.matched.length + " bones driven" : "static (no recognised body bones)"}`);
  ...
  if (vrm) {
    ...
    spring = buildSpringBones(model, { exclude: resolved.springExclude, regionWeight: profileFor(curKey).regions || {} });   // per-region jiggle weights restored from profile
      ...
      try {
        ...
        if (groups.length) {
          ...
          if (never.length) {
            ...
            spring = buildSpringBones(model, { exclude: resolved.springExclude, regionWeight: profileFor(curKey).regions || {}, neverExtra: _springNeverExtra });
    ...
    if (spring?.count && proc?.restAdjust && Object.keys(proc.restAdjust).length) {
      ...
      if (fixed) {
        model.updateWorldMatrix(true, true);
        spring = buildSpringBones(model, { exclude: resolved.springExclude, regionWeight: profileFor(curKey).regions || {}, neverExtra: _springNeverExtra });   // keep the twin-dedup exclusions through the rebuild
  ...
  // load had the squat-bound feet line wrong instead (shadow at her shins) — this fixes both.
  {
    ...
    if (shifted && spring?.count) spring = buildSpringBones(model, { exclude: resolved.springExclude, regionWeight: profileFor(curKey).regions || {}, neverExtra: _springNeverExtra });   // verlet tips re-capture from the shifted rest — no load-time settle wiggle
...
let _onboarding = false, _noModelEl = null;
function _noModelOverlay() {
  ...
  d.style.cssText = [
    ...
    "max-width:340px", "padding:18px 22px", "box-sizing:border-box",
    "font:14px/1.5 system-ui,sans-serif", "color:#eef1f8", "text-align:center",
    "background:rgba(24,26,34,0.92)", "border:1px solid rgba(255,255,255,0.18)",
    "border-radius:12px", "z-index:2147483646", "pointer-events:none",
    "box-shadow:0 6px 24px rgba(0,0,0,0.45)",
  ].join(";");
  d.innerHTML = "<div style='font-weight:600;margin-bottom:6px'>No model loaded</div>"
...
function clearOnboarding() {                                     // a real model loaded -> retract the message
...
function loadModel(url, label) {
  ...
  if (url === DEFAULT_KEY) { enterNoModel(); showOnboarding(); return; }   // #13: no-model state (inert marker + DOM message), NOT a self-made character
...
function spinLive(e) {   // live rotate-drag preview; no persist/footprint
  const r = _spinTo(e);
...
function updateLook(dt) {
  ...
  if ((lookOn || forced) && _cursorIdle < 2.5 && cursor.seen) {   // seen, not x>=0 — a cursor on a monitor LEFT of primary maps to negative local px and is still a real target
...
function animate() {
  ...
  const active = held || spinning || gliding || cursor.over || _lookW > 0.01 || voice.isSpeaking() || ui.isOpen() || performance.now() < _wakeUntil;   // _lookW: keep head-tracking smooth (not steppy at deep-rest)
...
const voice = createVoice({
  getFacial: () => facial,
  onSpeakStart: (dur) => { facial?.blink?.(); if (proc?.setLayer) proc.setLayer("cospeech", { fn: (t) => coSpeechPose(t, _speechRms) }); wake((+dur > 0 ? +dur : 2) + 0.5); },   // #22/#8: blink on speech ONSET (facial no longer free-runs blinks); P2: co-speech BODY layer from the live envelope
  onEnvelope: (rms) => { _speechRms += (rms - _speechRms) * 0.3; },   // smooth the RMS so the body emphasis doesn't twitch frame-to-frame
  onSpeakEnd: () => { _speechRms = 0; if (proc?.clearLayer) proc.clearLayer("cospeech"); },
  setStatus,
});
...
const EnigmaAvatar = {
  ...
  state: () => ({ held, size: +sizeScale.toFixed(2), dims: [+(modelDims.w || 0).toFixed(2), +(modelDims.h || 0).toFixed(2)], pos: [+pos.x.toFixed(2), +pos.y.toFixed(2)], screen: [innerWidth, innerHeight], screenPos: posScreen(), cursorPx: [cursor.x | 0, cursor.y | 0], over: cursor.over, vrm: !!vrm, procBones: proc ? proc.matched : [], layers: proc && proc.layerIds ? proc.layerIds() : [], springBones: spring ? spring.names : [], facial: facial ? { mode: facial.mode, info: facial.info } : null, attachments: attachObjs.map((a) => ({ id: a.id, category: a.category, attachedTo: a.attachedTo })), toggles: { spring: springOn, facial: facialOn, look: lookOn, locked, rotateMode, menu: ui.isOpen() } }),
  springTune: (p) => springTune(p),                                      // saved per-avatar (hair flow, etc.)
  ball: (action) => {                                                   // rapier ball-physics toys: throw / drop / clear. NOT a gesture catalog (purged) — body motion is AI-authored via pose / flex / perform.
    const n = String(action || "").toLowerCase().replace(/[ _-]/g, "");
  ...
  poseLayer: (c = {}) => {                                               // AI compositor: set ONE motion layer {parts,flex,dur,weight,env,id}; {clear:true} removes it
    if (!proc?.setLayer) return { error: "no procedural rig on this model" };
    const id = String(c.id || "ai_pose");
    if (c.clear) { proc.clearLayer(id); return { cleared: id }; }
    if (!c.parts && !c.flex) return { error: "pose needs parts:{role:[pitch,yaw,roll]} and/or flex:{role:[ang,abd]}" };
    proc.setLayer(id, { parts: c.parts, flex: c.flex, weight: c.weight, amp: c.amp, speed: c.speed, dur: c.dur, env: c.env });
    wake((+c.dur > 0 ? +c.dur : 1.6) + 0.4);
    return { posed: id, roles: [...Object.keys(c.parts || {}), ...Object.keys(c.flex || {})], layers: proc.layerIds() };
  },
  layer: (c = {}) => {                                                   // AI compositor: manage the layer stack — op add|clear|clearAll
    if (!proc?.setLayer) return { error: "no procedural rig on this model" };
    const op = String(c.op || (c.clear ? "clear" : "add")).toLowerCase();
    if (op === "clearall") { proc.clearLayers(); return { layers: [] }; }
    if (op === "clear") { if (!c.id) return { error: "layer clear needs id" }; proc.clearLayer(String(c.id)); return { layers: proc.layerIds() }; }
  ...
  fingers: (c = {}) => {                                                 // AI per-finger hand pose: {side:"L"|"R"|"both", curl: number(all) | {thumb|index|…|0..N: 0..1, default?} | null(release to grip)}
  ...
  conjure: (c = {}) => {                                                 // P3: make an object appear and move it (transform-based)
    if (c.clear) { conjurer.clear(); return { conjured: [] }; }
    if (c.dismiss) { conjurer.dismiss(String(c.dismiss)); return { dismissed: c.dismiss, ids: conjurer.ids() }; }
    if (c.move && c.to) { conjurer.moveTo(String(c.move), c.to, { dur: c.dur }); return { moved: c.move }; }
    const url = resolvePropName(c.url, CONJURE_ASSETS);
    if (!url) return { error: "conjure needs a .glb path or a known prop name (e.g. 'ball') | move+to | dismiss | clear" };
  ...
  perform: (text) => {                                                   // P4 substrate: tagged speech -> motion + the clean line to TTS
    const { clean, tags } = parseControlTags(text);
    const did = [];
    for (const { type, arg } of tags) {
      if (type === "conjure" && arg) { const url = resolvePropName(arg, CONJURE_ASSETS); if (url) { conjurer.spawn(url, { bone: "right_hand", dur: 8, float: 0.05 }); wake(2); did.push("conjure:" + arg); } else did.push("conjure-skip:" + arg + " (unknown prop - pass a .glb path or a known name)"); }
...
function answerQuery(what) {
  ...
  if (what === "rotation") {                                                  // LIVE rig rotation (a live rotate-drag differs from the saved profile — the driver must see the truth)
    const R = 180 / Math.PI;
    return { x: _norm360(rig.rotation.x * R), y: _norm360(rig.rotation.y * R), z: _norm360(rig.rotation.z * R), saved: getRot() };
  }
...
function handleCommand(c) {
  ...
  else if (c.action === "ball") return EnigmaAvatar.ball(c.name ?? c.value);   // rapier ball-physics toy: throwball / dropball / clearballs
  else if (c.action === "say" && c.url) EnigmaAvatar.say(c.url, c);     // play speech wav + lip-sync (+talk body language)
  else if (c.action === "mouth") EnigmaAvatar.mouth(c.value);           // manual jaw drive (testing) — coerced+guarded in EnigmaAvatar.mouth
  else if (c.action === "blink") { const n = +c.value; if (c.value != null && isFinite(n)) facial?.setBlink?.(n); else facial?.blink?.(); }   // finite value HOLDS the lids (wink/squint/calibrate; <0 resumes auto); no/garbage value = ONE quick blink
  else if (c.action === "setMorph") EnigmaAvatar.setMorph(c.index ?? c.idx ?? 0, c.value);   // probe a morph by index (find the mouth)
  ...
  else if (c.action === "lookAt") return EnigmaAvatar.lookAt(c.px, c.py);                // force gaze at a screen point — returns {drove} so the caller sees if it actually moved (#18)
  ...
  else if (c.action === "pose") return EnigmaAvatar.poseLayer(c);          // AI compositor: set a motion layer {parts,flex,dur,weight,env,id} (clear:true removes)
  else if (c.action === "layer") return EnigmaAvatar.layer(c);             // AI compositor: layer stack op add|clear|clearAll
  else if (c.action === "fingers" || c.action === "hand") return EnigmaAvatar.fingers(c);   // AI per-finger hand pose (fist / point / count / "no" wag)
  else if (c.action === "capabilities" || c.action === "caps") return EnigmaAvatar.capabilities();   // what the brain can drive on THIS model
  else if (c.action === "conjure") return EnigmaAvatar.conjure(c);          // P3: spawn/move/dismiss/clear a conjured prop
  else if (c.action === "perform") return EnigmaAvatar.perform(c.text);     // P4: drive motion from inline-tagged LLM speech; returns the clean line to speak
...
function applyPeerModel(url) {
  if (url === "__default__") { if (curKey !== DEFAULT_KEY) { enterNoModel(); showOnboarding(); } }   // #13: peer mirrors the no-model state too
...
window.avatarIPC?.onModel?.((url) => {   // peers mirror the brain's current model
  ...
  if (!_preloadDone) { _pendingModel = url; return; }   // don't resolve a rig before profiles are in
  applyPeerModel(url);
});
...
const UI_CMDS = {
  ...
  _rotLive:         { scope: "brain", fn: (r) => { if (r) applyRotationTo({ x: _norm360(r.x), y: _norm360(r.y), z: _norm360(r.z) }); } },   // rotate-drag preview from a peer (no persist — pointerup sends the real setRot)
  ...
  if (cursor.over) {
    ...
    if (!locked) {
      if (e.altKey || rotateMode) {   // ALT+drag rotates (↔ yaw, ↕ pitch) — any window. A held MODE hijacked the primary gesture ("can't move her, can rotate"; 2026-06-11) → a modifier can't get stuck; rotateMode remains for deliberate AI/bus use.
        spinning = true; _spinX = cursor.x; _spinY = cursor.y; _spinRot = getRot(); _downX = cursor.x;
...
function startup() {
  const seq = ++_loadSeq;
  const placeholder = () => { if (seq !== _loadSeq) return; console.warn("[avatar] no usable model -> no-model state (add a .glb)"); enterNoModel(); showOnboarding(); };   // #13: inert marker + DOM message, not a self-made character
