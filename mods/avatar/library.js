...
// avatars. Every model is just a user-supplied folder under models/; the original launch default
// is the procedural avatar (default_avatar.js) until a bespoke 3D avatar is made. Kept as an empty
// map so isBuiltin() stays a harmless no-op and the discovery/label code paths are unchanged.
const BUILTIN = {};
...
const isBuiltin = (id) => Object.prototype.hasOwnProperty.call(BUILTIN, id);
...
function createLibrary({ modelsDir, manifestPath, runPython = null, scriptDir = __dirname } = {}) {
  ...
  // is a model: label from the manifest (a user rename) → BUILTIN map → prettified folder name.
  function discoverModels() {
    ...
    for (const id of dirs) {
      ...
      if (!mesh) continue;
      out.push({
        id, builtin: isBuiltin(id),
        label: labels[id] || BUILTIN[id] || title(id),
        url: modelUrl(id, mesh),
        thumb: fs.existsSync(path.join(modelsDir, id, ".thumb.png")) ? thumbUrl(id) : null,
      });
    }
    out.sort((a, b) => (a.builtin === b.builtin ? a.label.localeCompare(b.label) : a.builtin ? -1 : 1));
    return out;
  }
  ...
  // A slug that is NOT a built-in and does NOT already exist — so an import can never clobber a
  // protected built-in OR a different user model that happens to share a generic name (scene.glb).
  function freeSlug(base) {
    if (!isBuiltin(base) && !fs.existsSync(path.join(modelsDir, base))) return base;
    for (let i = 2; i < 1000; i++) { const c = `${base}_${i}`; if (!isBuiltin(c) && !fs.existsSync(path.join(modelsDir, c))) return c; }
  ...
  function importFiles(files, opts = {}) {
    ...
    if (pkg) {
      ...
      if (isBuiltin(base)) return { error: "name collides with a built-in model" };
  ...
  return { meshInDir, discoverModels, registerModel, renameModel, importFiles, removeModel, freeSlug, isBuiltin, safeId, slug, title, modelUrl, thumbUrl, MESH_EXT, BUILTIN };
}

module.exports = { createLibrary, MESH_EXT, BUILTIN, SKIP_DIR, slug, title, isBuiltin, safeId, modelUrl, thumbUrl };
