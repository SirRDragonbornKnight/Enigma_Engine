...
const extOf = (url) => { const u = url.split(/[?#]/)[0].toLowerCase(); const i = u.lastIndexOf("."); return i >= 0 ? u.slice(i) : ""; };
...
export async function applyFbxMaterials(root, dir, mgr) {
  ...
  root.traverse((o) => { if (o.isMesh && o.material) wantsTextures = true; });
  if (!dir) {                                         // only disk loads have a sidecar
  ...
  try {
    ...
    if (!r.ok) {
      if (wantsTextures) problems.push(new Error(`no materials.json next to FBX (HTTP ${r.status}) -- materials render untextured`));
      return problems;
    }
    spec = await r.json();
  } catch (e) {
  ...
  if (wantsTextures && bound === 0) problems.push(new Error("materials.json matched none of the FBX material names -- nothing bound"));
  return problems;
}
