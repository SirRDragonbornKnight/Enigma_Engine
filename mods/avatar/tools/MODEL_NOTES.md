...
**mesh + skeleton** and *sometimes* **morph targets (blendshapes)** — and **drops everything
...
There is **no embedded toggle menu** in any current model, because that menu is a *Unity* thing.
...
- Mal0's **cloth is a mesh with no cloth bones** → it can't sway via bone-springs; that needs a
  vertex cloth sim (not built). The Cloth weight slider drives cloth *bones* when a model has them
...
- **marie** — **BROKEN: 147/149 bone names are U+FFFD mojibake** → 0 roles, no springs, no overrides possible (can't address bones by name); 29 of 41 meshes are rigid props. Fix = UTF-8 re-export, out of engine scope.
...
- MAJORITY: rigged (10/12); ≥16 roles (7/10 rigged); name tier does ~all role work (geometry tier filled only 5 roles across the library; rig_overrides.json is EMPTY — never needed yet).
