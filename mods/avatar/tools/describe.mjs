...
import {
  ...
  discoverModels, urlKeyFor, loadOverrides, MODELS,
} from "./rig_report.mjs";
...
// What avatar actions will actually DO something on this model. Heuristic + honest:
// final motion FEEL still needs a live look (see STATUS.md), so flag the uncertain ones.
function supportedActions(cascade, face, mats, springs) {
  const r = new Set(cascade.matched);
  const express = { supported: [], conditional: {}, unsupported: [] };
  // head-driven gestures
  (r.has("head") ? express.supported : express.unsupported).push("nod", "shake", "alert");
  // body-language gestures need a torso + limbs
  const bodied = cascade.matched.length >= 8;
  (bodied ? express.supported : express.unsupported).push("happy", "talk", "sad");
  // tail wag = hips-yaw in procedural.js → works whenever 'hips' resolves; the TAIL
  // only visually swings if spring physics catch a tail chain (name or geometry).
  if (r.has("hips")) express.supported.push("wag");
  else express.conditional.wag = "needs a 'hips' role to swing";
...
export function profile(file, overrides) {
  ...
  // names themselves are the failure (mojibake makes the name tier AND overrides useless).
  const corrupt = snap.filter((b) => b.name.includes(REPLACEMENT)).length;
  const notes = [];
  let ok = true;
  if (snap.length && corrupt / snap.length > 0.1) { ok = false; notes.push(`${corrupt}/${snap.length} bone names are corrupted (U+FFFD mojibake) — name-tier and rig_overrides can't address them; needs a UTF-8 re-export`); }
  ...
  const cascade = runCascade(gltf, snap, ov);
  ...
  // (the engine's OPEN_RE) → jaw BONE by /jaw/ → morphs present but UNNAMED (engine's
  // name picker finds nothing → needs an index override / geometric pick) → none.
  const OPEN_RE = /jaw.?open|mouth.?open|mouthopen|(^|[._-])aa($|[._-])|vrc\.v_aa|viseme.?aa/i;  // == facial.js
  ...
  const mouthOverride = ov?.face?.mouthMorph;
  let system, lipSyncViable, lipSyncNote;
  if (Number.isInteger(mouthOverride) && mouthOverride >= 0) { system = "morph(index-override)"; lipSyncViable = true; lipSyncNote = `mouth pinned to morph #${mouthOverride} via rig_overrides face.mouthMorph`; }
  else if (vrmExpr.length) { system = "vrm-expressions"; lipSyncViable = true; lipSyncNote = "VRM 'aa' viseme + blink"; }
  ...
  else if (morphs.total > 0) { system = "morph(unnamed)"; lipSyncViable = false; lipSyncNote = `${morphs.total} morph targets exist but NONE are named like a mouth shape. The live engine runs GEOMETRIC jaw-drop detection at load to find the mouth automatically (name-free) — confirm on the live overlay; override with rig_overrides face.mouthMorph if it picks wrong.`; }
  ...
  const face = { system, lipSyncViable, lipSyncNote, vrmExpressions: vrmExpr, morphTargets: morphs, jawBone };
  ...
  return {
    ...
    rig: {
      ...
      isVRM: cascade.isVRM,
      bySource: cascade.bySource,             // how each role was found (vrm/name/geometry/override)
...
function brief(file, overrides) {
  const p = profile(file, overrides);
...
function main() {
  ...
  if (!arg) {
    ...
    const menu = models.map((f) => brief(f, overrides));
  ...
  process.stdout.write(JSON.stringify(profile(file, overrides), null, 2) + "\n");
}
