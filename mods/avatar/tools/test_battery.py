...
async def behavior(bus, url):
    ...
    await bus.cmd({"action": "pose", "flex": {"right_arm": [1.1], "right_forearm": [0.6]}, "dur": 2.0, "id": "bat"}); await asyncio.sleep(1.2)
    await bus.cmd({"action": "fingers", "side": "R", "curl": 1.0}); await asyncio.sleep(0.8)
    await bus.cmd({"action": "fingers", "side": "R", "spec": {"default": 1, "index": 0}}); await asyncio.sleep(0.8)
    await bus.cmd({"action": "fingers", "side": "R", "curl": None}); await asyncio.sleep(0.6)
    ...
    await bus.cmd({"action": "ball", "name": "throwball"}); await asyncio.sleep(4)
    r["ball"] = await bus.snap("bat_ball.png")
    await bus.cmd({"action": "ball", "name": "clearballs"}); await asyncio.sleep(0.5)
