...
function clampToUnion(x, y) {
  const ds = displays();
  for (const d of ds) {                          // inside ANY display (bottom PERMEABLE at 1.4x) -> valid as-is; never fight a cross-bezel drag
    const b = d.bounds;
  ...
  return { x: Math.max(b.x + m, Math.min(b.x + b.width - m, x)), y: Math.max(b.y + m, Math.min(b.y + b.height * 1.4, y)) };
}
...
function onDisplayMetricsChanged() {
  if (_rebuilding) { rebuildWindowSet(); return; }   // mid-rebuild → let rebuild coalesce it (its trailing pass settles the layout)
  ...
  if (!same || live.length !== windows.length) { rebuildWindowSet(); return; }   // set changed (or a window died) → rebuild
  // Same set: resize each window to its display's current bounds in place (no reload).
  for (const w of live) {
    const d = displays().find((x) => x.id === w.displayId);
    if (!d) { rebuildWindowSet(); return; }   // a display vanished between the check and here → rebuild
    const b = d.bounds;
    try { w.win.setBounds({ x: b.x, y: b.y, width: b.width, height: b.height }); } catch {}
  }
...
async function importModel() {
  const res = await dialog.showOpenDialog(brainWin(), {
    ...
    filters: [
      ...
      { name: "Model + assets together", extensions: ["glb", "gltf", "vrm", "fbx", "bin", "png", "jpg", "jpeg", "tga"] },
