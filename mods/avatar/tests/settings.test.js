...
function weightInRow(text) {
  for (const span of S().querySelectorAll("span")) if (span.textContent.includes(text) && span.parentElement) { const n = span.parentElement.querySelector("input[type=number]"); if (n) return n; }
  return null;
}
...
test("chain on/off checkbox: set a CUSTOM weight, uncheck → 0, recheck → restores the CUSTOM weight", () => {
  ...
  try {
    ...
    n.value = "1.6"; fire(n, "input");
    assert.ok(calls.some((c) => c[0] === "setRegionWeight" && c[1] === "breast" && Math.abs(c[2] - 1.6) < 1e-6), "typing 1.6 → setRegionWeight('breast', 1.6)");
...
function fitBoneSelect() {
  for (const span of S().querySelectorAll("span")) if (span.textContent.trim() === "Bone" && span.parentElement) { const s = span.parentElement.querySelector("select"); if (s) return s; }
  return null;
}
...
test("attach Bone picker reflects RESOLVED roles + (world / no bone), not a hardcoded list", () => {
  ...
  try {
    ...
    const { api } = makeApi({
      getAttachObjs: () => [{ id: "a1", category: "prop", url: "./props/wand.glb", bone: "right_hand", pos: [0, 0, 0], rot: [0, 0, 0], scale: 1, obj: null }],
    });
    createUI(api).showSettings();
    ...
    assert.strictEqual([...bsel.options].find((o) => o.value === "").textContent, "(world / no bone)", "no-bone option labeled");
...
test("attach Bone picker keeps a non-role bone the AI named (e.g. 'tail') as the selection", () => {
  ...
  try {
    const { api, calls } = makeApi({
      getAttachObjs: () => [{ id: "a1", category: "prop", url: "./props/bow.glb", bone: "tail", pos: [0, 0, 0], rot: [0, 0, 0], scale: 1, obj: null }],
    });
    createUI(api).showSettings();
    const bsel = fitBoneSelect();
    assert.ok(bsel, "attach Bone <select> exists");
    ...
    bsel.value = "head"; fire(bsel, "change");
    assert.ok(calls.some((c) => c[0] === "tuneAttachment" && c[1] === "a1" && c[2]?.bone === "head"), "changing the bone -> tuneAttachment({bone:'head'})");
  } finally { dom.cleanup(); }
});
...
test("attach Bone picker offers only (world / no bone) when no roles resolve", () => {
  ...
  try {
    const { api } = makeApi({
      getRoleInfo: () => ({ matched: 0, total: 19, missing: [
      ...
      getAttachObjs: () => [{ id: "a1", category: "furniture", url: "./props/chair.glb", bone: "", pos: [0, 0, 0], rot: [0, 0, 0], scale: 1, obj: null }],
    });
    createUI(api).showSettings();
    const bsel = fitBoneSelect();
    assert.ok(bsel, "attach Bone <select> exists");
...
test("menu 'Ball' submenu lists the ball-physics toys and clicking one fires api.ball", () => {
  ...
  try {
    ...
    assert.ok(calls.some((c) => c[0] === "ball" && c[1] === "throwball"), "clicking Throw ball fires api.ball('throwball')");
  } finally { dom.cleanup(); }
});
...
test("Settings panel header drag moves it by the EXACT pointer delta", () => {
  ...
  try {
    ...
    panel.getBoundingClientRect = () => ({ left: 500, top: 400, right: 768, bottom: 600, width: 268, height: 200, x: 500, y: 400 });
