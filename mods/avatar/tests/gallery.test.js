...
test("Add-to-avatar Prop... clicks through importProp -> attachMesh", async () => {
  ...
  try {
    ...
    prop.click();
    await Promise.resolve(); await Promise.resolve();            // let importProp() resolve, then attachMesh runs
    assert.ok(made.calls.some((c) => c[0] === "importProp"), "Prop... -> avatarIPC.importProp()");
