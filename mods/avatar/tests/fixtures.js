...
export function namedQuadruped() {
  ...
  const legD = (name, x, z) => makeBone(name, [x, 0, z], [
  ...
  const hips = makeBone("Hips", [0, 1.0, 0], [
    ...
    legD("FrontLeftLeg", -0.3, 0.5), legD("FrontRightLeg", 0.3, 0.5),
    legD("BackLeftLeg", -0.3, -0.5), legD("BackRightLeg", 0.3, -0.5),
  ]);
  return underArmature(hips);
}
...
export function minimalNameBiped() {
  const armC = (pfx, sx) => makeBone(pfx + "Shoulder", [sx * 0.05, 0.10, 0], [
    makeBone(pfx + "UpperArm", [sx * 0.13, 0, 0], [
      makeBone(pfx + "Forearm", [sx * 0.26, 0, 0], [
        makeBone(pfx + "Hand", [sx * 0.22, 0, 0], [makeBone(pfx + "Hand_end", [sx * 0.08, 0, 0])]),
      ]),
    ]),
  ]);
  // thigh "L_Leg", shin "L_Leg2" (numeric-suffix gap), foot "L_Foot".
  const legC = (sx, side) => makeBone(side + "_Leg", [sx * 0.10, -0.05, 0], [
    makeBone(side + "_Leg2", [0, -0.42, 0], [
  ...
  const hips = makeBone("Hips", [0, 1.00, 0], [
    makeBone("Spine", [0, 0.12, 0], [
      makeBone("Chest", [0, 0.14, 0], [
        makeBone("Neck", [0, 0.16, 0], [makeBone("Head", [0, 0.10, 0], [makeBone("Head_end", [0, 0.18, 0])])]),
        armC("Left", -1), armC("Right", +1),
      ]),
    ]),
    legC(-1, "L"), legC(+1, "R"),
  ]);
