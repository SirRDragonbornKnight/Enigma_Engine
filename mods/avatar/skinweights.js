...
export function computeWeightMass(model) {
  ...
  model.traverse((o) => {
    ...
    const idx = o.geometry.getAttribute("skinIndex"), w = o.geometry.getAttribute("skinWeight");
...
export function subtreeMass(root, mass) {
  let m = 0;
  root.traverse((o) => { if (o.isBone) m += mass.get(o) || 0; });
  return m;
}

const _pa = new THREE.Vector3(), _pb = new THREE.Vector3(), _da = new THREE.Vector3(), _db = new THREE.Vector3();
const _boneChild = (b) => { for (const c of b.children) if (c.isBone) return c; return null; };
...
export function coincident(a, b, tol) {
  a.getWorldPosition(_pa); b.getWorldPosition(_pb);
  if (_pa.distanceTo(_pb) > tol) return false;
  const ca = _boneChild(a), cb = _boneChild(b);
  if (!ca || !cb) return _pa.distanceTo(_pb) <= tol * 0.5;
  ca.getWorldPosition(_da).sub(_pa); cb.getWorldPosition(_db).sub(_pb);
...
export function findRoleTwins(roleBones, mass, modelHeight) {
  ...
  let total = 0; for (const v of mass.values()) total += v;
  const minMass = Math.max(0.5, total * 5e-4);
  const related = (x, y) => { for (let p = x; p; p = p.parent) if (p === y) return true; for (let p = y; p; p = p.parent) if (p === x) return true; return false; };
  const root = roles[0] ? (() => { let r = roles[0]; while (r.parent && r.parent.isBone) r = r.parent; return r; })() : null;
  const all = [];
  (root ? root.parent || root : null)?.traverse?.((o) => { if (o.isBone) all.push(o); });
  if (!all.length) for (const [b] of mass) all.push(b);          // fallback: whatever the mass map saw
  for (const role in roleBones) {
    const R = roleBones[role]; if (!R) continue;
    for (const B of all) {
...
export function groupCoincidentRoots(roots, modelHeight) {
  ...
  for (let i = 0; i < roots.length; i++) {
    ...
    for (let j = i + 1; j < roots.length; j++) {
      ...
      if (coincident(roots[i], roots[j], tol)) { g.push(roots[j]); taken.add(roots[j]); }
    }
