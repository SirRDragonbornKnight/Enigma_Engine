...
export function buildProceduralRig(model, boneLimits = {}, resolved = null) {
  ...
  const fingers = { L: [], R: [] };                     // per hand: [{ name, joints:[{b,rest,ax(local),depth}], cur, tgt }] — tgt=null follows the reactive grip
  {
    ...
    const FINGER_RE = [["thumb", /thumb/i], ["index", /index|fore|point/i], ["middle", /middle|mid/i], ["ring", /ring/i], ["pinky", /pinky|little|pink/i]];
    const nameOf = (root) => { const n = root.name || ""; for (const [lab, re] of FINGER_RE) if (re.test(n)) return lab; return null; };
    const axisFor = (o, q0) => {                         // the proven per-joint curl-axis calibration; null = leaf (caller inherits)
      let tip = o, bestDepth = -1;
    ...
    const calibrate = (handRole, key) => {
      ...
      let auto = 0;
      for (const root of hand.children) {                // each direct bone-child of the hand = ONE finger
        ...
        const walk = (o) => {
          ...
          let ax = axisFor(o, q0);
          if (!ax) {                                     // leaf joint → inherit parent's axis, converted into this leaf's frame
            const pe = joints.find((e) => e.b === o.parent);
            if (pe) { const pw = new THREE.Quaternion(); o.parent.getWorldQuaternion(pw); const lw = new THREE.Quaternion(); o.getWorldQuaternion(lw); ax = pe.ax.clone().applyQuaternion(pw).applyQuaternion(lw.invert()).normalize(); }
          }
    ...
    if (fingers.L.length + fingers.R.length) console.log(`[avatar] fingers: L[${fingers.L.map((f) => f.name).join(",")}] R[${fingers.R.map((f) => f.name).join(",")}] (${nj("L")}+${nj("R")} joints; curl axes calibrated toward the palm)`);
  }
  ...
  const clampFlex = (role, ang) => {
    const L = limits[role]; if (!L) return Math.max(-FLEX_CAP, Math.min(FLEX_CAP, ang || 0));
    const lo = (L.pitch_min ?? -126) * DEG, hi = (L.pitch_max ?? 126) * DEG;
    return Math.max(lo, Math.min(hi, ang || 0));
  };
  const clampAbd = (role, abd) => {
    const L = limits[role]; if (!L) return Math.max(-FLEX_CAP, Math.min(FLEX_CAP, abd || 0));
    const lim = Math.max(Math.abs((L.yaw_min ?? 0)), Math.abs((L.yaw_max ?? 0)),
                         Math.abs((L.roll_min ?? 0)), Math.abs((L.roll_max ?? 0))) * DEG || FLEX_CAP;
  ...
  let _additive = false, lookX = 0, lookY = 0, lookW = 0;
  ...
  const _lq = new THREE.Quaternion(), _le = new THREE.Euler();
  ...
  function applyLayers(dt) {
    ...
    _acc.clear();
    const acc = (role) => { let e = _acc.get(role); if (!e) { e = { p: 0, y: 0, r: 0, ang: 0, abd: 0, hasParts: false, hasFlex: false }; _acc.set(role, e); } return e; };
    ...
    for (const [id, L] of layers) {
      ...
      if (L.dur > 0) {
        if (localT >= L.dur) { layers.delete(id); continue; }   // timed layer expired -> drop it
      ...
      if (parts) for (const role in parts) {
        ...
        const o = parts[role], e = acc(role);
        e.p += (o[0] || 0) * w; e.y += (o[1] || 0) * w; e.r += (o[2] || 0) * w; e.hasParts = true;
      }
      if (flexes) for (const role in flexes) {
        if (!bones[role]) continue;
        const o = flexes[role], e = acc(role);
        e.ang += (o[0] || 0) * w; e.abd += (o[1] || 0) * w; e.hasFlex = true;
      }
    }
    ...
    for (const [role, e] of _acc) {
      ...
      if (e.hasFlex) {
        ...
        if (ax) {
          ...
          if (e.abd && abductAxis[role]) { v0.abd = _vclamp(v0.abd, clampAbd(role, e.abd), maxStep); b.quaternion.multiply(_lq.setFromAxisAngle(abductAxis[role], v0.abd)); }
          else v0.abd = 0;
        } else {
    ...
    for (const [role, v0] of _vstate) {
      ...
      const b = bones[role]; if (!b) { _vstate.delete(role); continue; }
      const maxStep = speedLimit(role) * dt;
      v0.p = _vclamp(v0.p, 0, maxStep); v0.y = _vclamp(v0.y, 0, maxStep); v0.r = _vclamp(v0.r, 0, maxStep);
      v0.ang = _vclamp(v0.ang, 0, maxStep); v0.abd = _vclamp(v0.abd, 0, maxStep);
      if (!v0.p && !v0.y && !v0.r && !v0.ang && !v0.abd) { _vstate.delete(role); continue; }
      if (v0.p || v0.y || v0.r) { _le.set(v0.p, v0.y, v0.r, "XYZ"); b.quaternion.multiply(_lq.setFromEuler(_le)); }
      const ax = flexAxis[role];
      if (v0.ang) { if (ax) b.quaternion.multiply(_lq.setFromAxisAngle(ax, FSIGN * v0.ang)); else { _le.set(v0.ang, 0, 0, "XYZ"); b.quaternion.multiply(_lq.setFromEuler(_le)); } }
  ...
  function update(dt, walk = false, opts = {}) {
    ...
    if (_additive) { applyLayers(dt); return; }
    ...
    flex("left_arm", 0.035, 0.03); flex("right_arm", 0.035, 0.03);   // the static A-pose hang — a fixed POSE, not motion
  ...
  return {
    ...
    setLayer: (id, spec) => {             // AI compositor: add/replace a motion layer (spec=null clears it)
      if (!id) return false;
      const k = String(id);
    ...
    capabilities: () => ({                // what the brain can drive on THIS resolved model
      roles: Object.keys(bones),
      flexRoles: Object.keys(flexAxis),
      channels: { pose: true, flex: Object.keys(flexAxis).length > 0, look: true, layers: true, fingers: { L: fingers.L.map((f) => f.name), R: fingers.R.map((f) => f.name) } },
      limits: limits,
      units: { offsets: "radians", flex: "radians", limits: "degrees" },   // pose/flex offsets are RADIANS; the limits table above is in DEGREES
      fsign: FSIGN,
    }),
    setGrip: (side, on) => { const v = on ? 0.4 : 0; if (side === "L" || side === "both") gripTgtL = v; if (side === "R" || side === "both") gripTgtR = v; },   // reactive carry grip — a GENTLE hold (fingers with no explicit setFingers target follow this)
    ...
    setFingers: (side, spec) => {
      ...
      for (const k of ks) {
        const arr = fingers[k];
        if (spec == null) { for (const f of arr) f.tgt = null; continue; }   // release -> reactive grip
        const num = typeof spec === "number";
        const def = num ? spec : (spec && spec.default != null ? +spec.default : null);
        arr.forEach((f, i) => {
          let v = def;
          if (!num && spec && typeof spec === "object") { if (spec[f.name] != null) v = +spec[f.name]; else if (spec[i] != null) v = +spec[i]; }
    ...
    bindExtras: (x) => {                                 // avatar.js hands over the sprung-bone names AFTER springs resolve
      ...
      for (const k of ["L", "R"]) { for (const f of fingers[k]) f.joints = f.joints.filter((j) => !sprung.has(j.b.name)); fingers[k] = fingers[k].filter((f) => f.joints.length); }
    },
    ...
    gripState: () => ({ grip: [+gripL.toFixed(2), +gripR.toFixed(2)], fingers: { L: fingers.L.map((f) => ({ name: f.name, curl: +f.cur.toFixed(2), tgt: f.tgt })), R: fingers.R.map((f) => ({ name: f.name, curl: +f.cur.toFixed(2), tgt: f.tgt })) }, reach: +armReachLive().toFixed(3) }),   // DIAGNOSTIC: live per-finger curl + targets
    ...
    jointAngles: () => {   // DIAGNOSTIC: live joint angles from WORLD positions (180=straight, <180=bent) — unambiguous
      ...
      return { leftKnee: ang("left_leg", "left_shin", "left_foot"), leftElbow: ang("left_arm", "left_forearm", "left_hand"), handGap: gap, armReach: +armReach.toFixed(3), fsign: FSIGN, fwd: [+_forward.x.toFixed(2), +_forward.y.toFixed(2), +_forward.z.toFixed(2)], roles: Object.keys(bones).length };
    },
