...
📍 FILE: enigma_engine/comms/api_server.py
...
🚀 HOW TO START:
    python run.py --serve
    # OR
    python -m enigma_engine.comms.api_server
...
📖 SEE ALSO:
    • enigma_engine/comms/network.py       - Multi-device networking
    • enigma_engine/comms/remote_client.py - Connect to remote servers
    • enigma_engine/comms/mobile_api.py    - Mobile app API
"""
import logging
import secrets
...
def require_api_key(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # Check for API key
        api_key = CONFIG.get("Enigma AI Engine_api_key")
        if not api_key:
...
def create_app():
    ...
    def info():
        ...
        """
        api_key_configured = CONFIG.get("Enigma AI Engine_api_key") is not None
    ...
    def generate_key():
        ...
        if CONFIG.get("Enigma AI Engine_api_key"):
            return jsonify({
        ...
        return jsonify({
            ...
            "example": f"export Enigma AI Engine_API_KEY={new_key}"
        })
