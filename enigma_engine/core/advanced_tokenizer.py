"""
Forge Tokenizer - Custom BPE Implementation
=============================================

The Enigma AI Engine's native tokenizer, built from scratch with unique features.

Differentiators from GPT-style tokenizers:
  - [E:token] format - Enigma AI Engine's unique special token syntax
...
Core Features:
  ...
  - Compatible with Forge trainer interface

This is the Enigma AI Engine's custom tokenizer - not a GPT clone.
"""
import json
import logging
import random
...
class ForgeTokenizer:
    """
    Enigma AI Engine's Native Tokenizer.

    Custom BPE implementation with unique features:
    - [E:token] format for special tokens (Enigma AI Engine's signature)
    - Multi-modal support built-in
    - Tool invocation and reasoning tokens
    - Optimized for both code and natural language
    """

    # Enigma AI Engine's custom regex pattern for pre-tokenization
    ...
    def __init__(self, vocab_file: Optional[Path] = None):
        ...
        # Enigma AI Engine's unique [E:token] format - distinguishes from GPT's <|token|>
        self.special_tokens = {
            # Core tokens
            "[E:pad]": 0,
            "[E:start]": 1,
            ...
            "[E:user]": 8,
            "[E:forge]": 9,  # The AI's name, not generic "bot"
            "[E:assistant]": 10,
            "[E:system]": 11,
            ...
            # Meta tokens - Enigma AI Engine's reasoning system
            "[E:think]": 33,
            "[E:/think]": 34,
            "[E:err]": 35,
            "[E:warn]": 36,
            ...
            # Enigma AI Engine-specific tokens
    ...
    def get_info() -> dict[str, Any]:
        """
        Get information about the Enigma AI Engine Tokenizer.
        ...
        return {
            'name': 'ForgeTokenizer',
            'version': '2.1.0',
            'engine': 'Enigma AI Engine',
            'type': 'Byte-level BPE',
            'description': "Enigma AI Engine's native tokenizer with unique [E:token] format",
            'features': [
            ...
            'differentiators': [
                ...
                'Native Enigma AI Engine integration',
                'Optimized for both code and natural language',
            ],
        }
    ...
    def __repr__(self) -> str:
        ...
        return f"ForgeTokenizer(vocab_size={self.vocab_size}, merges={len(self.bpe_ranks)})"
    
    def __str__(self) -> str:
        """Return a human-readable string."""
        return f"ForgeTokenizer v2.1.0 - Vocab: {self.vocab_size:,} tokens"
...
def train_tokenizer(
    ...
    """
    Train an Forge tokenizer on data files.
