...
def _ensure_imports() -> bool:
    """Load torch + transformers on first call. Returns True if available."""
    global _HAVE_TRANSFORMERS, torch, AutoModelForCausalLM, AutoTokenizer
    global TextIteratorStreamer, BitsAndBytesConfig
    if _HAVE_TRANSFORMERS is not None:
    ...
    try:
        ...
        from transformers.generation.streamers import (
            TextIteratorStreamer as _Streamer,
        )
...
class _LazyFlag:
    ...
    def __bool__(self):
        return _ensure_imports()
    def __repr__(self):
        return f"HAVE_TRANSFORMERS({_ensure_imports()})"

HAVE_TRANSFORMERS = _LazyFlag()
...
def convert_huggingface_to_forge(
    ...
    **kwargs
    ...
    try:
        ...
        raise ValueError(f"Weight conversion failed: {e}") from e
