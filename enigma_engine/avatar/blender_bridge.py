...
Allows Enigma AI Engine to control 3D models in Blender in real-time:
...
Usage:
    # In Enigma AI Engine
    from enigma_engine.avatar.blender_bridge import BlenderBridge
...
class BlenderBridgeConfig:
    ...
    # Expression mappings (Enigma AI Engine expression -> Blender shape keys)
...
BLENDER_ADDON_CODE = '''
"""
Enigma AI Engine Blender Addon

Install this in Blender: Edit > Preferences > Add-ons > Install
Then enable "Enigma AI Engine Avatar Bridge"

Creates a socket server that Enigma AI Engine connects to for real-time control.
"""

bl_info = {
    "name": "Enigma AI Engine Avatar Bridge",
    "author": "Enigma AI Engine",
    "version": (1, 0, 0),
    "blender": (3, 0, 0),
    "location": "View3D > Sidebar > Enigma AI Engine",
    "description": "Real-time avatar control from Enigma AI Engine",
    "category": "Animation",
}
...
def handle_command(cmd_data):
    """Handle a command from Enigma AI Engine."""
...
def server_thread():
    ...
    print(f"[Enigma AI Engine] Server listening on port {_port}")
    
    while _running:
        try:
            _client_socket, addr = _server_socket.accept()
            print(f"[Enigma AI Engine] Client connected: {addr}")
            ...
            if _client_socket:
                ...
                _client_socket = None
            print("[Enigma AI Engine] Client disconnected")
    ...
    print("[Enigma AI Engine] Server stopped")


class Enigma AI Engine_OT_start_server(bpy.types.Operator):
    bl_idname = "Enigma AI Engine.start_server"
    bl_label = "Start Server"
    bl_description = "Start Enigma AI Engine connection server"
...
class Enigma AI Engine_OT_stop_server(bpy.types.Operator):
    bl_idname = "Enigma AI Engine.stop_server"
    bl_label = "Stop Server"
    bl_description = "Stop Enigma AI Engine connection server"
...
class Enigma AI Engine_PT_panel(bpy.types.Panel):
    bl_label = "Enigma AI Engine Avatar Bridge"
    bl_idname = "Enigma AI Engine_PT_panel"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_category = 'Enigma AI Engine'
    ...
    def draw(self, context):
        ...
        if _running:
            ...
            layout.operator("Enigma AI Engine.stop_server", icon='PAUSE')
        else:
            layout.label(text="Status: Stopped", icon='X')
            layout.operator("Enigma AI Engine.start_server", icon='PLAY')
...
classes = [
    Enigma AI Engine_OT_start_server,
    Enigma AI Engine_OT_stop_server,
    Enigma AI Engine_PT_panel,
]
...
def save_blender_addon(output_dir: Optional[str] = None) -> str:
    ...
    addon_path = out_path / "Enigma AI Engine_blender_addon.py"
