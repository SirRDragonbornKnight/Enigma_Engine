...
📍 FILE: enigma_engine/memory/vector_db.py
...
🔗 CONNECTED FILES:
    → USES:      enigma_engine/memory/embeddings.py (convert text to vectors)
    ← USED BY:   enigma_engine/memory/manager.py (ConversationManager)
    ← USED BY:   enigma_engine/memory/rag.py (retrieval-augmented generation)

📖 SEE ALSO:
    • enigma_engine/memory/manager.py    - Uses this for conversation search
    • enigma_engine/memory/embeddings.py - Converts text to vectors
    • enigma_engine/memory/search.py     - High-level search interface
"""
