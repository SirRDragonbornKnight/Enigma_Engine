...
In the quiet depths of Enigma AI Engine lies the Listener's Chamber, where spoken
...
FILE: enigma_engine/voice/listener.py
...
MAIN CLASSES: VoiceListener, VoiceConfig
    ...
    [Ambient silence...]
    User: "Hey Enigma AI Engine!"        <- Wake word detected!
    [Listener awakens]
    User: "What's the weather?"  <- Speech captured
    [Text sent to AI]

WAKE WORD OPTIONS:
    "Hey Enigma AI Engine" - The primary invocation
...
CONNECTED REALMS:
    PROVIDES TO:    enigma_engine/gui/ - Voice input for chat
    WORKS WITH:     enigma_engine/voice/voice_generator.py - Complete voice I/O
    USES:           speech_recognition library (Google Speech API)

USAGE:
    from enigma_engine.voice.listener import VoiceListener, VoiceConfig
...
class VoiceConfig:
    ...
    def __post_init__(self):
        if self.wake_words is None:
            self.wake_words = ["hey ai tester", "enigma_engine", "hey engine", "a"]
...
class VoiceCommander:
    ...
    def __init__(self, command_callback: Callable[[str], None] = None):
        ...
        self.current_model = "Enigma AI Engine"
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            self._notify("Voice Enabled", 
                        f"Say 'Hey Enigma AI Engine' to activate.\n"
    ...
    def _on_listening(self, is_listening: bool):
        ...
        if is_listening:
            ...
            self._status("Ready (say 'Hey Enigma AI Engine')")
