...
Handles clean shutdown of Enigma AI Engine components when the application exits.
...
FILE: enigma_engine/utils/shutdown.py
TYPE: System Utility
MAIN CLASSES: ShutdownManager

USAGE:
    from enigma_engine.utils.shutdown import get_shutdown_manager, on_shutdown
...
class ShutdownManager:
    ...
    Coordinates cleanup across all Enigma AI Engine components by:
...
def _register_builtin_cleanups():
    """Register cleanup for core Enigma AI Engine components."""
    manager = get_shutdown_manager()
    
    # Module manager cleanup (high priority - save state)
    def cleanup_modules():
        try:
            from enigma_engine.modules.manager import get_manager
            mgr = get_manager()
    ...
    def cleanup_voice():
        try:
            from enigma_engine.voice.voice_pipeline import _pipeline
            if _pipeline:
                _pipeline.stop()
        except Exception as e:
    ...
    def cleanup_api():
        try:
            from enigma_engine.web.server import shutdown_server
            shutdown_server()
        except Exception as e:
