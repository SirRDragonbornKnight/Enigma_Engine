...
Privacy-preserving distributed learning for Enigma AI Engine where devices train locally and share only model improvements (weight updates), never raw data.

## Overview

The federated learning system allows multiple Enigma AI Engine instances to collaboratively improve their models without sharing private training data. Each device:
...
```python
from enigma_engine.learning import DataFilter
...
```python
from enigma_engine.learning import TrainingCoordinator
...
```python
from enigma_engine.comms.discovery import discover_federated_learning_peers
...
```python
from enigma_engine.gui.widgets.federated_widget import FederatedWidget
...
Part of Enigma AI Engine - See LICENSE file for details.
