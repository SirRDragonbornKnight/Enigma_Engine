...
def _save_api_keys(parent):
    ...
    keys_to_save = {
        ...
        "ANTHROPIC_API_KEY": parent.anthropic_key_input.text().strip(),
...
def _change_robot_mode(parent):
    ...
    try:
        from enigma_engine.tools.robot_modes import RobotMode, get_mode_controller
...
def _toggle_robot_camera(parent, state):
    ...
    try:
        from enigma_engine.tools.robot_modes import CameraConfig, get_mode_controller
...
def _move_to_monitor(parent, monitor_index):
    ...
    # The window will open on this monitor next time Enigma AI Engine starts
    main_window = parent.window()
...
def _apply_system_prompt_preset(parent):
    ...
    FULL_PROMPT = """You are Enigma AI Engine, an intelligent AI assistant with access to various tools and capabilities.
...
Be friendly, concise, and proactive in helping users accomplish their goals."""
    ...
    ENIGMA_ENGINE_FULL_PROMPT = """You are the AI assistant for Enigma AI Engine, a modular AI framework. You have access to many capabilities.

## Enigma AI Engine Features You Can Use
...
Ready to assist with coding, creativity, learning, or just chatting!"""
    ...
    if preset == "simple":
        ...
        """)
    elif preset == "enigma_engine_full":
        parent.custom_system_prompt.setReadOnly(True)
        parent.custom_system_prompt.setText(ENIGMA_ENGINE_FULL_PROMPT)
        parent.custom_system_prompt.setStyleSheet("""
            QTextEdit {
                background-color: #252525;
                border: 1px solid #444;
...
def create_settings_tab(parent):
    ...
    parent.startup_position_combo.setToolTip(
        "Where the main window appears when Enigma AI Engine starts\n"
    ...
    parent.anthropic_key_input.setText(_get_env_key("ANTHROPIC_API_KEY"))
    anthropic_row.addWidget(parent.anthropic_key_input)
    api_layout.addLayout(anthropic_row)
...
def _apply_theme(parent):
    ...
    try:
        ...
        QMessageBox.warning(
            ...
            "Please ensure the enigma_engine.gui.theme_system module is properly installed."
        )
    except Exception as e:
...
System:
  ...
  GPU Available: No"""
        ...
        parent.power_status.setPlainText(
            ...
            "Make sure enigma_engine.core.power_mode module exists."
        )
    except Exception as e:
