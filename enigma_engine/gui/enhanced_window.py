...
class EnhancedMainWindow(QMainWindow):
    ...
    def _on_send(self):
        ...
        attachment_html = ""
        try:
            from .tabs.chat_tab import get_attachments, format_attachments_html, process_attachments_for_ai, _clear_attachments
            attachments = get_attachments(self)
            if attachments:
                attachment_html = format_attachments_html(attachments)
        except Exception:
            pass  # Attachment handling not critical
        
        # Allow sending with just attachments (no text required if there are attachments)
        ...
        user_name = getattr(self, 'user_display_name', 'You')
        message_text = text if text else "(sent attachments)"
        user_html = f'''<div style="background-color: #313244; padding: 8px; margin: 4px 0; border-radius: 8px; border-left: 3px solid #89b4fa;">
            <b style="color: #89b4fa;">{user_name}:</b> {message_text}
            {attachment_html}
        </div>'''
        self.chat_display.append(user_html)
        self.chat_input.clear()
        
        # Clear attachments after adding to chat
        if attachments:
            try:
        ...
        # Track user message (optionally with attachment info)
        message_record = {
            "role": "user",
            "text": text,
            "ts": time.time()
        ...
        # Process images for vision AI if available
        processed_attachments = None
        if attachments:
            try:
                processed_attachments = process_attachments_for_ai(attachments)
                
                # If there are images and we have vision capability, add context
                if processed_attachments.get('images'):
                    # Try to describe images using vision tool
                    try:
                        from ..tools.vision_tools import analyze_image
                        for img_path in processed_attachments['images'][:3]:  # Limit to 3 images
                            description = analyze_image(img_path)
                            if description:
                                text = f"{text}\n\n[Image attached: {description}]" if text else f"[Image: {description}]"
                    except ImportError:
                        # Vision tools not available, just note the attachment
                        text = f"{text}\n\n[{len(processed_attachments['images'])} image(s) attached]" if text else f"[{len(processed_attachments['images'])} image(s) attached]"
                    except Exception:
                        pass  # Vision analysis failed, continue without
            except Exception:
                pass  # Attachment processing failed, continue without
