...
class AITesterConfig:
    ...
    @classmethod
    def from_dict(cls, d: Dict[str, Any]) -> 'AITesterConfig':
        known = {
...
MODEL_PRESETS = {
    ...
    'nano': AITesterConfig(dim=128, n_layers=4, n_heads=4, n_kv_heads=2, max_seq_len=256),
    'micro': AITesterConfig(dim=192, n_layers=4, n_heads=4, n_kv_heads=2, max_seq_len=384),

    # Edge / Raspberry Pi (~5-15M params)
    'tiny': AITesterConfig(dim=256, n_layers=6, n_heads=8, n_kv_heads=4, max_seq_len=512),
    'mini': AITesterConfig(dim=384, n_layers=6, n_heads=6, n_kv_heads=3, max_seq_len=512),

    # Consumer GPU (~27-85M params)
    'small': AITesterConfig(dim=512, n_layers=8, n_heads=8, n_kv_heads=4, max_seq_len=1024),
    'medium': AITesterConfig(dim=768, n_layers=12, n_heads=12, n_kv_heads=4, max_seq_len=2048),
    'base': AITesterConfig(dim=896, n_layers=14, n_heads=14, n_kv_heads=2, max_seq_len=2048),

    # Prosumer GPU (~200M-600M params)
    'large': AITesterConfig(dim=1024, n_layers=16, n_heads=16, n_kv_heads=4, max_seq_len=4096),
    'xl': AITesterConfig(dim=1536, n_layers=24, n_heads=24, n_kv_heads=6, max_seq_len=4096, dropout=0.05),

    # Multi-GPU / Server (~1B-3B params)
    'xxl': AITesterConfig(dim=2048, n_layers=32, n_heads=32, n_kv_heads=8, max_seq_len=8192, dropout=0.05),
    'huge': AITesterConfig(dim=2560, n_layers=40, n_heads=40, n_kv_heads=8, max_seq_len=8192, dropout=0.05),

    # Datacenter / Cloud (~7B-13B params)
    'giant': AITesterConfig(dim=4096, n_layers=32, n_heads=32, n_kv_heads=8, max_seq_len=8192, dropout=0.05),
    'colossal': AITesterConfig(dim=4096, n_layers=48, n_heads=32, n_kv_heads=8, max_seq_len=16384, dropout=0.05),

    # Maximum Scale (~30B+ params)
    'titan': AITesterConfig(dim=6144, n_layers=48, n_heads=48, n_kv_heads=12, max_seq_len=16384, dropout=0.05),
    'omega': AITesterConfig(dim=8192, n_layers=64, n_heads=64, n_kv_heads=16, max_seq_len=32768, dropout=0.05),
}
...
def get_preset(name: str, vocab_size: int = 8000) -> AITesterConfig:
...
def estimate_parameters(config: AITesterConfig) -> int:
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, config: AITesterConfig, layer_id: int):
        super().__init__()
...
class AITester(nn.Module):
    ...
    def __init__(
        ...
        max_len: Optional[int] = None, config: Optional[AITesterConfig] = None, **kwargs
    ):
        super().__init__()
        ...
        if config is not None:
            ...
            self.config = AITesterConfig(
                ...
                **{k: v for k, v in kwargs.items() if hasattr(AITesterConfig, k)}
            )
    ...
    def from_config(cls, config: Dict[str, Any]) -> 'Enigma':
        return cls(config=AITesterConfig.from_dict(config))
...
def create_model(size: str = 'small', vocab_size: int = 8000, **kwargs) -> AITester:
    ...
    print(f"Created Enigma ({size}): {model.num_parameters:,} params, "
