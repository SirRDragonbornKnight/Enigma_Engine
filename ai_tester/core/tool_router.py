...
USAGE:
    ...
    router.assign_model("chat", "enigma:small_ai_tester")
...
class ModelAssignment:
    ...
    model_type: str  # "ai_tester", "huggingface", "local", "api"
...
class ToolRouter:
    ...
    def _set_defaults(self):
        ...
        self.assignments = {
            "chat": [ModelAssignment("enigma:default", "ai_tester", priority=10)],
            "image": [ModelAssignment("local:stable-diffusion", "local", priority=10)],
            "code": [ModelAssignment("enigma:default", "ai_tester", priority=10)],
    ...
    def load_model(self, model_id: str) -> Any:
        ...
        model_type, model_name = model_id.split(":", 1) if ":" in model_id else ("ai_tester", model_id)
        
        model = None
        
        if model_type == "ai_tester":
        ...
        try:
            ...
            if name == "default":
                ...
                if models_dict:
                    ...
                    name = next(iter(models_dict.keys()), "small_ai_tester")
                else:
                    return None
                    
            model, config = registry.load_model(name)
            return {"model": model, "config": config, "type": "ai_tester"}
        except Exception as e:
    ...
    def _execute_text_generation(self, assignment: ModelAssignment, 
        ...
        if model_data["type"] == "ai_tester":
            # Use Enigma inference
            try:
                from .inference import AITesterEngine
                engine = AITesterEngine(model_data["model"], model_data["config"])
