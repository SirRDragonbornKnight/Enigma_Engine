...
USAGE:
  ...
  node = AITesterNode(name="pc_node", port=5000)
  node.start_server()
  
  # On Pi (client mode)
  from ai_tester.comms.network import AITesterNode
  node = AITesterNode(name="pi_node")
    ...
    def engine(self):
        ...
        if self._engine is None:
            if self.model_name:
                ...
                self._engine = AITesterEngine.__new__(AITesterEngine)
...
def create_server_node(name: str, port: int = 5000, model_name: str = None) -> AITesterNode:
...
def create_client_node(name: str, server_url: str) -> AITesterNode:
...
if __name__ == "__main__":
    ...
    print("="*40)
    print("\nServer mode:")
    print("  node = AITesterNode('my_server', port=5000)")
    print("  node.start_server()")
    print("\nClient mode:")
    print("  node = AITesterNode('my_client')")
