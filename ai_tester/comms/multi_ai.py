"""
Multi-AI Communication System for Enigma
...
Usage:
    from ai_tester.comms.multi_ai import AIConversation
    
    # Create conversation between two models
    conv = AIConversation()
    conv.add_participant("enigma_1", model_name="ai_tester")
    conv.add_participant("enigma_2", model_name="ai_tester")
...
class AIParticipant:
    ...
    def __init__(
        ...
        Args:
            ...
            remote_url: URL of remote Enigma node (for network mode)
        ...
        if self._engine is None and self.model_name:
            try:
                ...
                if self.model_name in registry.registry.get("models", {}):
                    ...
                    self._engine = AITesterEngine(model_path=model_path)
                else:
                    # Use default model
                    self._engine = AITesterEngine()
    ...
    def _remote_respond(self, prompt: str) -> str:
        """Get response from remote Enigma node."""
        try:
...
class AIConversation:
    ...
    def add_participant(
        ...
        Args:
            ...
            remote_url: Remote Enigma URL (network mode)
...
def quick_ai_chat(
    prompt: str,
    model1: str = "ai_tester",
    model2: str = "ai_tester",
    num_turns: int = 3
) -> List[Dict]:
    """
