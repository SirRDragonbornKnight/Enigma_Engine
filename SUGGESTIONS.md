...
0c. **ARCH-V1d** (Qwen3 architecture support). After V1c, llama.cpp's `llama` arch silently rejects 4 QK-norm tensors per layer (Enigma's `use_qk_norm=True` is the default), producing `done_getting_tensors: wrong number of tensors`. Fix: when QK-norm tensors are present, emit `general.architecture=qwen3` plus the Qwen3-specific metadata schema. Bounded — touches `GGUFExporter._infer_metadata` and `GGUFMetadata.to_dict`.
0d. **ARCH-V1e** (real tokenizer encoding). Even after V1d, generation crashes because `tokenizer.ggml.model="llama"` requires SentencePiece BPE merges that we don't emit. Fix: emit a real BPE merges array if the tokenizer is BPE, OR use a no-vocab variant for byte-level test models. This is the last gap before the round-trip xfails can flip to passing.
