...
- `python -m pytest tests/ -q` → **3018 passed, 3 skipped**

ARCH-1.5c migration status (code verified):

- Forge launchers now dispatcher-backed for `sft` (`018c8f9`), `dpo` (`22ae19a`), `grpo` (`3e61f4b`), `remax` (`0c7cc0b`), `vision` (`388050e`), `lora` (`fbf2976`), `rlhf` (`9bf88b9`), `self_play` (`9bf88b9`), and the `simpo`/`orpo` preference variants (Pass 156z9bz).
- ARCH-1.5c remains open for: RLHF/ReMax phase-1 reward-model paths (still direct `RewardTrainer(...)`), the queue worker (`gui_forge_queue.py`), the LoRA non-PEFT fallback (`gui_forge_training.py`), and the meta-mode/composition launchers (`adaptive`, `distill`, `dialogue`, `evolutionary`, `pretrain`) which still instantiate `Trainer(...)` directly. Audio launcher work remains scoped to ARCH-1d because no Forge audio training launcher/button exists in current GUI sources.
...
1. Run `python -m pytest tests/ -q` once and confirm baseline still matches **3018 passed, 3 skipped**.
