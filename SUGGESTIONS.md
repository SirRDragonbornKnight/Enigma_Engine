...
**Last updated:** April 30, 2026 (Pass 156z9g — Rule #20 audit closed all four findings: dead `Power mode` comments killed in `inference.py`; `MemoryConfig` shrunk from 6 fields to the 1 with a real consumer + 5 on-disk profile JSONs migrated; AutoResearch-2 wired into `/api/chat` (pre+post) and `/api/chat/stream` (pre only) for parity with the GUI; `is_roleplay()` docstring tense reframed to name its live consumer; GGUF dequant Park entry added below.)
**Tests:** 2624 passed (+4 new `TestAutoResearchAPIParity`), 9 skipped, 0 failures.
**Rust:** unchanged — 9/9.
