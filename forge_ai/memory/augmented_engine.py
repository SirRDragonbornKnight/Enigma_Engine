...
📍 FILE: forge_ai/memory/augmented_engine.py
...
🔗 CONNECTED FILES:
    → USES:      forge_ai/core/inference.py (ForgeEngine)
    → USES:      forge_ai/memory/vector_db.py (SimpleVectorDB)
    → USES:      forge_ai/memory/embeddings.py (EmbeddingGenerator)
    → USES:      forge_ai/memory/rag.py (RAGSystem)
    ← USED BY:   forge_ai/gui/tabs/chat_tab.py (via generate_with_memory)
"""
...
class MemoryAugmentedEngine:
    ...
    📐 USAGE:
        from forge_ai.memory.augmented_engine import MemoryAugmentedEngine
