"""
Local Network Discovery for ForgeAI

Uses mDNS/Bonjour to advertise ForgeAI service on the local network,
...
class LocalDiscovery:
    """
    Advertise ForgeAI service on local network using mDNS/Bonjour.
    
    Makes ForgeAI discoverable as "ForgeAI on [ComputerName]" in network browsers.
    """
    ...
    def advertise(self, port: int, version: str = "1.0") -> bool:
        """
        Advertise ForgeAI service on local network.
        ...
        try:
            ...
            # Create service info
            service_name = f"ForgeAI on {self._hostname}"
            service_type = "_forgeai._tcp.local."
            ...
            logger.info(f"ForgeAI service advertised on {local_ip}:{port}")
            return True
    ...
    def stop(self):
        ...
        if self.zeroconf and self.service_info:
            try:
                ...
                self.zeroconf.close()
                logger.info("Stopped advertising ForgeAI service")
            except Exception as e:
