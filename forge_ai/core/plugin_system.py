"""
Plugin System for Forge_AI

Allows extending Forge_AI with custom:
- Inference backends
- Training methods
- Model architectures
- Tools/capabilities
...
1. forge_ai/plugins/ directory
2. User plugins directory (~/.forge/plugins/)
3. Installed packages with 'forge_ai.plugins' entry point
"""
...
class ForgePlugin(ABC):
    """
    Base class for all Forge_AI plugins.
...
class PluginManager:
    ...
    def __init__(self, plugin_dirs: Optional[list[Path]] = None):
        ...
        self._plugin_dirs = plugin_dirs or [
            Path(__file__).parent.parent / "plugins",  # forge_ai/plugins/
    ...
    def _discover_from_entry_points(self) -> list[str]:
        ...
        try:
            if sys.version_info >= (3, 10):
                ...
                eps = entry_points(group='forge_ai.plugins')
            else:
                from importlib.metadata import entry_points
                eps = entry_points().get('forge_ai.plugins', [])
