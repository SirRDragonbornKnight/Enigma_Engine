...
Export converts ForgeAI models to GGUF format for Ollama.
...
class OllamaProvider(ExportProvider):
    ...
    def _create_modelfile(
        ...
        description = metadata.get("description", f"ForgeAI model: {model_name}")
...
SYSTEM \"\"\"You are {model_name}, a helpful AI assistant created with ForgeAI. You provide clear, accurate, and helpful responses.\"\"\"
...
# License
LICENSE \"\"\"MIT License - Model trained with ForgeAI\"\"\"
"""
...
ForgeAI model packaged for Ollama.
...
class OllamaImporter(ImportProvider):
    ...
    This importer downloads models and registers them in ForgeAI.
    ...
    def import_model(
        ...
        **kwargs
        ...
        """
        Pull an Ollama model and register it in ForgeAI.
        
        Args:
            source_id: Ollama model name (e.g., "llama2", "mistral:7b")
            local_name: Local name in ForgeAI registry
        """
        if not HAVE_OLLAMA:
            return ImportResult(
        ...
        try:
            ...
            # Create ForgeAI wrapper directory
