...
The ForgeAI's native tokenizer, built from scratch with unique features.

Differentiators from GPT-style tokenizers:
  - [E:token] format - ForgeAI's unique special token syntax
...
class ForgeTokenizer:
    ...
    def __init__(self, vocab_file: Optional[Path] = None):
        ...
        # ForgeAI's unique [E:token] format - distinguishes from GPT's <|token|>
        self.special_tokens = {
            # Core tokens
            "[E:pad]": 0,
            "[E:start]": 1,
            ...
            # Meta tokens - ForgeAI's reasoning system
            "[E:think]": 33,
            "[E:/think]": 34,
            "[E:err]": 35,
            "[E:warn]": 36,
    ...
    def get_info() -> dict[str, Any]:
        ...
        return {
            ...
            'engine': 'ForgeAI',
            'type': 'Byte-level BPE',
            'description': "ForgeAI's native tokenizer with unique [E:token] format",
            'features': [
