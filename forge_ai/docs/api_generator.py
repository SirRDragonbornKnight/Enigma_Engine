...
FILE: forge_ai/docs/api_generator.py
TYPE: Documentation
...
class APIDocGenerator:
    ...
    def _document_function(self, name: str, func: Callable, module: str) -> FunctionDoc:
        ...
        # Build parameters
        params = []
        for param_name, param in (sig.parameters.items() if hasattr(sig, 'parameters') else {}.items()):
            if param_name == 'self':
                continue
        ...
        return FunctionDoc(
            ...
            parameters=params,
            return_type=hints.get('return', Any).__name__ if 'return' in hints else "",
    ...
    def export_markdown(self, output_path: Path):
        ...
        """
        lines = []
        lines.append("# ForgeAI API Reference\n")
    ...
    def export_openapi(self, output_path: Path, title: str = "ForgeAI API", version: str = "1.0.0"):
