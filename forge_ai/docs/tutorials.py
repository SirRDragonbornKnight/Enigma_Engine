...
Jupyter notebook template generator for ForgeAI tutorials.
Creates guided learning experiences with code, explanations, and exercises.

FILE: forge_ai/docs/tutorials.py
TYPE: Documentation
...
class TutorialGenerator:
    ...
    def generate_quickstart(self) -> Path:
        ...
        config = TutorialConfig(
            title="ForgeAI Quickstart",
            level=TutorialLevel.BEGINNER,
            description="Get up and running with ForgeAI in minutes.",
        ...
        nb.add_section("Installation", "Let's start by installing ForgeAI.")
        nb.add_code("!pip install forge-ai")
        
        nb.add_section("Import", "Import the main ForgeAI module.")
        nb.add_code("""import forge_ai
from forge_ai.core import ForgeEngine

print(f"ForgeAI version: {forge_ai.__version__}")""")
...
# Generate a response
response = engine.generate("Hello, ForgeAI!")
print(response)""")
        
        nb.add_exercise(
            "Try Custom Prompts",
            "Modify the prompt below to ask ForgeAI a question.",
...
""")
    ...
    def generate_training_tutorial(self) -> Path:
        ...
        config = TutorialConfig(
            title="Training Models with ForgeAI",
...
Training starts with good data. ForgeAI accepts plain text files
...
print("Training data saved!")""")
        ...
        nb.add_code("""from forge_ai.core import TrainingConfig, Trainer
...
# What happens with different settings?""",
    ...
    def generate_modules_tutorial(self) -> Path:
        ...
        config = TutorialConfig(
            title="ForgeAI Module System",
            level=TutorialLevel.INTERMEDIATE,
            description="Master the modular architecture of ForgeAI.",
...
ForgeAI uses a modular architecture where every capability is a toggleable module.
This prevents conflicts and allows flexible configuration.
""")
        
        nb.add_code("""from forge_ai.modules import ModuleManager
...
print(response)""")
    ...
    def _generate_index(self, tutorials: list[Path]) -> Path:
        ...
        config = TutorialConfig(
            title="ForgeAI Tutorials",
            description="Interactive tutorials for learning ForgeAI."
        )
