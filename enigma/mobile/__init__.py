"""
Mobile Package for Enigma Engine

This module re-exports from enigma.comms.mobile_api for backwards compatibility.

The main mobile API implementation lives in enigma.comms.mobile_api.

Usage:
    from enigma.mobile import MobileAPI, run_mobile_api
    
    # Run API server
    run_mobile_api(host='0.0.0.0', port=5001)
    
    # Or create instance with custom settings
    api = MobileAPI(port=5001, model_name='enigma')
    api.run()
