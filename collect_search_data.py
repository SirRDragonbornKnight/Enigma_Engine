...
_POSITIVE_EXAMPLES: list[tuple[str, str]] = [
    ...
    ("Who won the 2024 US presidential election?",
     "<search>2024 US presidential election winner</search>"),
    ("What was the closing price of TSLA yesterday?",
     "<search>TSLA stock closing price yesterday</search>"),
    ("Who is the current CEO of OpenAI?",
     "<search>current CEO of OpenAI</search>"),
    ("What is the latest stable release of Python?",
     "<search>latest Python stable release version</search>"),
    ("Who won the most recent Super Bowl?",
     "<search>most recent Super Bowl winner</search>"),
    ("What major news happened this week in AI research?",
     "<search>recent AI research news this week</search>"),
    ("What is the current weather in Tokyo?",
     "<search>current weather Tokyo</search>"),
    ("What were the headlines on BBC News this morning?",
     "<search>BBC News headlines today</search>"),
    # Specific verifiable facts (numbers, names, places)
    ("What is the population of Lagos as of 2024?",
     "<search>population of Lagos 2024</search>"),
    ("How tall is the Burj Khalifa in meters?",
     "<search>Burj Khalifa height meters</search>"),
    ("What is the GDP of Vietnam?",
     "<search>GDP of Vietnam</search>"),
    ("When was the James Webb Space Telescope launched?",
     "<search>James Webb Space Telescope launch date</search>"),
    ("Who directed the film Parasite?",
     "<search>director of Parasite 2019 film</search>"),
    ("What is the chemical formula for caffeine?",
     "<search>chemical formula of caffeine</search>"),
    ...
    ("Who wrote the song 'Bohemian Rhapsody'?",
     "<search>writer of Bohemian Rhapsody</search>"),
    # Technical / API / version queries
    ("What is the default port for PostgreSQL?",
     "<search>PostgreSQL default port</search>"),
    ...
    ("What HTTP status code means 'I'm a teapot'?",
     "<search>HTTP status code 418 teapot</search>"),
    ("What is the latest version of CUDA?",
     "<search>latest CUDA toolkit version</search>"),
    ("What command shows disk usage on Linux?",
     "<search>linux disk usage command</search>"),
    ("What is the current LTS version of Node.js?",
     "<search>current Node.js LTS version</search>"),
    # Lookups by name (people, papers, repos)
    ("What papers has Geoffrey Hinton published in 2024?",
     "<search>Geoffrey Hinton publications 2024</search>"),
    ("What does the GitHub repo 'karpathy/nanoGPT' contain?",
     "<search>karpathy nanoGPT github repository</search>"),
    ...
    ("What is the recommended max sequence length for Llama 3 8B?",
     "<search>Llama 3 8B max sequence length</search>"),
    ("What does RFC 9110 specify about HTTP semantics?",
     "<search>RFC 9110 HTTP semantics summary</search>"),
    ("What is the current PEP for structural pattern matching in Python?",
     "<search>PEP structural pattern matching Python</search>"),
    ("What dataset did the FineMath-3plus paper use?",
     "<search>FineMath 3plus dataset paper</search>"),
    ("What license does the Mistral 7B model release under?",
     "<search>Mistral 7B model license</search>"),
]
...
_NEGATIVE_EXAMPLES: list[tuple[str, str]] = [
    ...
    ("Why does ice float on water?",
     ...
     "making solid water less dense than liquid water."),
    ...
    ("Give me a name for a fluffy cat.",
     "How about Mochi? It suits a soft, round, cuddly cat."),
    ("Sort this list mentally: 3, 1, 4, 1, 5, 9, 2, 6.",
    ...
    ("How do you check if a key exists in a Python dict?",
     ...
     "This is O(1) and the idiomatic check."),
    ("What is the time complexity of inserting into a hash table?",
     "Average case is O(1); worst case is O(n) when many keys "
...
def _validate_examples() -> None:
    ...
    for prompt, completion in _POSITIVE_EXAMPLES:
        if "<search>" not in completion or "</search>" not in completion:
            bad.append(
                f"POSITIVE missing <search>...</search>: {prompt!r}"
            )
    ...
    if bad:
        raise RuntimeError(
            "synthetic search corpus has malformed examples:\n  "
            + "\n  ".join(bad)
        )
...
def build_corpus(
    ...
    *when* search is not warranted, otherwise it overfits to "always
    ...
    if positive_only and negative_only:
        raise ValueError(
            "--positive-only and --negative-only are mutually exclusive"
        )
    _validate_examples()
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--tag", default="seed",
        help="Output filename suffix (default: 'seed' → "
             "synthetic_search_seed.{jsonl,txt}).",
    )
    parser.add_argument(
    ...
    n_pos = sum(
        1 for p in pairs if "<search>" in p["completion"]
    )
    n_neg = len(pairs) - n_pos
    logger.info(
        "wrote %d rows (%d positive, %d negative) to %s and %s "
        "(%d text blocks)",
        n_jsonl, n_pos, n_neg, jsonl_path, text_path, n_text,
    )
    return 0
