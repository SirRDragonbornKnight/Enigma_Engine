...
print("=" * 70)
print("SKEPTICAL FULL SYSTEM TEST - Enigma AI Engine")
print("=" * 70)
...
def t8():
    from enigma_engine.core.model import MODEL_PRESETS, get_preset
...
def t35():
    from enigma_engine.avatar.presets import PresetManager
    return len(PresetManager.BUILTIN_PRESETS) > 0

@test("create_avatar_subtab function exists")
def t36():
    from enigma_engine.gui.tabs.avatar.avatar_display import create_avatar_subtab
...
def t40():
    from enigma_engine.gui.tabs.code_tab import CodeTab
    return CodeTab is not None

@test("QuickCommandOverlay class exists")
def t41():
    from enigma_engine.gui.system_tray import QuickCommandOverlay
...
def t48():
    ...
    from enigma_engine.core import model, tokenizer, inference, training, tool_router
    return True

@test("Paths use pathlib (spot check)")
def t49():
    from enigma_engine.core.training import MODELS_DIR, DATA_DIR
    return hasattr(MODELS_DIR, 'exists') and hasattr(DATA_DIR, 'exists')

@test("Logger defined in key modules")
def t50():
    import enigma_engine.core.model as m1
    import enigma_engine.core.inference as m2
    import enigma_engine.core.tool_router as m3
